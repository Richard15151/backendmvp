...
# ---------------ROTA PARA ALUNO VER PERFIL, USANDO JWT-----------------------
...
# ---------------ROTA PARA SALVAR TENTATIVA NOS JOGOS-----------------------
...
# ---------------ROTAS GET PARA VERIFICAR O BANCO-----------------------
...
#Rota para obter o histórico de desempenho de um aluno em uma trilha específica e jogo específico
@app.route('/api/alunos/<int:aluno_id>/historico/trilha/<int:trilha_id>/jogo/<int:jogo_id>', methods=['GET'])
def get_historico_aluno_por_trilha_e_jogo(aluno_id, trilha_id, jogo_id):
    aluno = Aluno.query.get(aluno_id)
    trilha = Trilha.query.get(trilha_id)
    jogo = Jogo.query.get(jogo_id)
    ...
    desempenhos = DesempenhoJogo.query.filter_by(aluno_id=aluno_id, trilha_id=trilha_id, jogo_id=jogo_id)\
                                 .options(db.joinedload(DesempenhoJogo.jogo))\
                                 .order_by(DesempenhoJogo.data_hora.desc())\
                                 .all()
